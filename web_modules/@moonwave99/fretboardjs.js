...
function getModeFromScaleType(type) {
    const { modeNum } = get$4(type.replace('pentatonic', '').trim());
    return modeNum;
}
...
class FretboardSystem {
    ...
    getScale({ type = 'major', root: paramsRoot = 'C', box }) {
        ...
        const { notes, empty, intervals } = get$3(scaleName);
        if (empty) {
...
class Fretboard {
    ...
    on(eventName, handler) {
        const { svg, options, strings, frets, hoverDiv, dots } = this;
        ...
        if (this.handlers[eventName]) {
            ...
            this.handlers[eventName] = throttle(THROTTLE_INTERVAL, (event) => handler(getPositionFromMouseCoords(Object.assign({ event,
                stringsGroup,
                strings,
                frets,
                dots }, options))));
...
export { Fretboard, GUITAR_TUNINGS, Systems };
